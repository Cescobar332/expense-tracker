  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingCat) {
      updateMutation.mutate({
        id: editingCat.id,
        data: { name: form.name, color: form.color, icon: form.icon || undefined },
      });
    } else {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {filtered.map((cat) => (
          <div
            key={cat.id}
            className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-4 flex items-center justify-between hover:shadow-sm transition-shadow"
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <span
                  className={`text-xs ${cat.type === 'INCOME' ? 'text-[var(--color-success)]' : 'text-[var(--color-danger)]'}`}
                >
            ...
            <div className="flex gap-1 flex-shrink-0">
              <button
                ...
                className="p-2 text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] rounded min-h-[40px] min-w-[40px] flex items-center justify-center"
              ...
              <button
                ...
                className="p-2 text-[var(--color-text-secondary)] hover:text-[var(--color-danger)] rounded min-h-[40px] min-w-[40px] flex items-center justify-center"
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {[
          ...
          >
            {tab.label}
            <span className="ml-1.5 text-xs opacity-75">({getTabCount(tab.value)})</span>
          </button>
        ))}
      </div>
      ...
      <Modal
        isOpen={showModal}
        onClose={closeModal}
        title={editingCat ? t['categories.editTitle'] : t['categories.newTitle']}
      >
            ...
            <div>
              ...
              <div id="category-type" className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`py-2 px-4 rounded-lg text-sm font-medium border transition-colors min-h-[44px] ${form.type === 'EXPENSE' ? 'bg-[var(--color-danger)]/10 border-[var(--color-danger)]/30 text-[var(--color-danger)]' : 'border-[var(--color-border)] text-[var(--color-text-secondary)]'}`}
                >
                  {t['common.expense']}
                </button>
                <button
                  ...
                  className={`py-2 px-4 rounded-lg text-sm font-medium border transition-colors min-h-[44px] ${form.type === 'INCOME' ? 'bg-[var(--color-success)]/10 border-[var(--color-success)]/30 text-[var(--color-success)]' : 'border-[var(--color-border)] text-[var(--color-text-secondary)]'}`}
                >
                  {t['common.income']}
                </button>
              </div>
