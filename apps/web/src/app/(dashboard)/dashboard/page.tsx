...
export default function DashboardPage() {
  ...
  const monthLabel = new Intl.DateTimeFormat(getLocale(lang), {
    month: 'long',
    year: 'numeric',
  }).format(selectedDate);
  ...
  const { data: allTimeReport } = useQuery({
    queryKey: ['report', 'all-time'],
    queryFn: () => reportsApi.getSummary({}),
  });

  // Filter savings: only show goals created on or before the selected month
  const selectedMonthEnd = new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0);
  const filteredSavings = savings?.filter((s) => new Date(s.startDate) <= selectedMonthEnd) || [];
  const totalSavings = filteredSavings.reduce((sum, s) => sum + Number(s.currentAmount), 0);
