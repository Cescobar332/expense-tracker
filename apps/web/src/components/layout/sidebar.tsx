...
const getNavItems = (t: Record<string, string>) => [
  ...
  { href: '/savings', label: t['nav.savings'], icon: (
...
export function Sidebar() {
  ...
  const languageMutation = useMutation({
    mutationFn: (newLang: string) => authApi.updateProfile({ language: newLang }),
    onSuccess: (_result, newLang) => {
      if (user) {
  ...
  const currentLanguageLabel = LANGUAGES.find((l) => l.value === (user?.language || 'es'))?.label || 'Espa\u00F1ol';
  ...
  const languagePickerUI = (
    <div className="mb-2">
      ...
      >
        <span>{t['sidebar.language']}: {currentLanguageLabel}</span>
        <svg className={`w-4 h-4 transition-transform ${showLanguagePicker ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="grid grid-cols-2 gap-1">
            {LANGUAGES.map((lang) => (
              <button
                key={lang.value}
                onClick={() => {
                  if (lang.value !== (user?.language || 'es') && !languageMutation.isPending) {
                    languageMutation.mutate(lang.value);
                  }
                }}
                disabled={languageMutation.isPending}
                className={`px-2 py-1.5 text-xs rounded transition-colors ${
                  lang.value === (user?.language || 'es')
                    ? 'bg-[var(--color-primary)] text-white'
                    : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-border)]'
                } ${languageMutation.isPending ? 'opacity-50 cursor-wait' : ''}`}
              >
                {lang.label}
              </button>
            ))}
  ...
  const currencyPickerUI = (
    <div className="mb-2">
      <button
        onClick={() => setShowCurrencyPicker(!showCurrencyPicker)}
        className="w-full flex items-center justify-between px-3 py-2 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-border)] rounded-lg transition-colors"
      >
        <span>{t['sidebar.currency']}: {user?.currency || 'USD'}</span>
        ...
        <div className="mt-1 p-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)]">
          ...
          {currencyMutation.isPending && (
            <p className="text-xs text-[var(--color-primary)] mt-2 text-center">
              {t['sidebar.convertingAmounts']}
            </p>
          )}
          {currencyMutation.isError && (
            <p className="text-xs text-[var(--color-danger)] mt-2 px-1">
              {currencyMutation.error instanceof Error ? currencyMutation.error.message : t['sidebar.currencyChangeError']}
            </p>
          )}
        </div>
      )}
