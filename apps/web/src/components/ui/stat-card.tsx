    ...
    <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] shadow-sm p-4 md:p-6 min-w-0 overflow-hidden">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-sm text-[var(--color-text-secondary)] truncate" title={title}>
            {title}
          </p>
          <p
            className="mt-1 text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-tight truncate"
            style={{ color }}
            title={value}
          >
            {value}
